...
export default function Gallery() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="bg-gradient-to-br from-purple-700 to-primary-800 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-white/70 font-semibold text-sm">School Memories</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black mb-3 tracking-tight">Gallery</h1>
            <p className="text-white/75 text-lg">
              Memories from Peter Harvard INT'L School
              {images.length > 0 && (
                <span className="ml-2 text-xs font-bold px-2 py-0.5 rounded-full bg-white/20">{images.length} photos</span>
              )}
            </p>
          </motion.div>
        </div>
      ...
      <AnimatePresence>
        {lightbox !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div key={lightbox} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }}
              className="flex flex-col items-center gap-4 max-w-[90vw]" onClick={e => e.stopPropagation()}>
