...
import {
  ...
  ClipboardList, CheckCircle, Search, Megaphone, LayoutDashboard,
  ChevronRight, MessageCircle, BookOpen, Images
} from 'lucide-react'
...
const fadeUp = { hidden: { y: 24, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.5, ease: 'easeOut' } } }
  ...
  useEffect(() => {
    ...
    async function loadStats() {
      ...
      setStats({
        ...
        years: new Set((yearsRes.data || []).map(a => a.graduation_year)).size,
      })
    }
    loadStats()
  }, [])
    ...
    <div className="overflow-x-hidden">
      ...
      <section className="relative min-h-[92vh] flex items-center bg-gradient-to-br from-primary-800 via-primary-700 to-primary-600 text-white overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 60" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
            <path d="M0 60L60 50C120 40 240 20 360 15C480 10 600 20 720 28C840 36 960 42 1080 40C1200 38 1320 28 1380 23L1440 18V60H0Z"
        ...
        >
          {[
            ...
            <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} viewport={{ once: true }}
              ...
              <div className={`w-11 h-11 rounded-xl ${s.bg} ${s.color} flex items-center justify-center`}>{s.icon}</div>
              <p className="text-3xl font-black text-gray-900 dark:text-white">{s.value}</p>
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 leading-tight">{s.label}</p>
            </motion.div>
          ))}
        </motion.div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { to: '/directory', icon: <Users size={20} />, label: 'Directory', color: 'bg-primary-600' },
            { to: '/events', icon: <CalendarDays size={20} />, label: 'Events', color: 'bg-blue-600' },
            { to: '/gallery', icon: <Images size={20} />, label: 'Gallery', color: 'bg-purple-600' },
            { to: '/blog', icon: <BookOpen size={20} />, label: 'Blog', color: 'bg-green-600' },
          ].map(l => (
      ...
      <section className="bg-gray-50 dark:bg-gray-900/40 border-y border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-black text-gray-900 dark:text-white mb-3">Everything you need</h2>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl font-black text-gray-900 dark:text-white flex items-center gap-3">
                <Star size={26} className="text-yellow-500 fill-yellow-500" /> Featured Alumni
              </h2>
              <p className="text-gray-500 dark:text-gray-400 mt-1">Spotlighting our notable graduates</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <Link to="/directory" className="inline-flex items-center gap-2 text-sm font-bold text-primary-600 dark:text-primary-400 hover:gap-3 transition-all">
        ...
        <section className="bg-gray-50 dark:bg-gray-900/40 border-y border-gray-100 dark:border-gray-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
              <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                <h2 className="text-3xl font-black text-gray-900 dark:text-white flex items-center gap-3">
                  <CalendarDays size={26} className="text-primary-500" /> Latest Events
                </h2>
                <p className="text-gray-500 dark:text-gray-400 mt-1">What's happening in your school community</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                <Link to="/events" className="inline-flex items-center gap-2 text-sm font-bold text-primary-600 dark:text-primary-400 hover:gap-3 transition-all">
            ...
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }}
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.map(e => (
                <motion.div key={e.id} variants={fadeUp}><EventCard event={e} /></motion.div>
              ))}
            </motion.div>
          </div>
        </section>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-primary-700 to-primary-900 py-24">
          ...
          <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <h2 className="text-4xl md:text-5xl font-black text-white mb-5">Ready to reconnect?</h2>
